...
//        ?tms=false disables storing/loading tiles in TMS-style coordinates.
...
function MBTiles(uri, callback) {
    ...
    if (uri.query.tms === undefined) uri.query.tms = true;
    if (typeof uri.query.tms === 'string') uri.query.tms = (uri.query.tms != 'false');
    if (uri.query.ro === undefined) uri.query.ro = false;
    if (typeof uri.query.ro === 'string') uri.query.ro = (uri.query.ro == 'true');

    var mbtiles = this;
    this.setMaxListeners(0);
    this.filename = uri.pathname;
    this._batchSize = +uri.query.batch;
    this._tms = !!uri.query.tms;
    this._readOnly = !!uri.query.ro;

    Step(function() {
        if (mbtiles._readOnly) {
            // SQLITE_OPEN_READONLY | SQLITE_OPEN_FULLMUTEX
            mbtiles._db = new sqlite3.Database(mbtiles.filename, sqlite3.OPEN_READONLY | 0x00010000, this);
        } else {
            // defaults from node-sqlite3 are:
...
MBTiles.prototype.startWriting = function(callback) {
    if (typeof callback !== 'function') throw new Error('Callback needed');
    if (!this.open) return callback(new Error('MBTiles not yet loaded'));
    if (this._readOnly) return callback(new Error('MBTiles opened read-only'));
